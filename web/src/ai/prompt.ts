...
4. 行星轮系固定写法：太阳轮为 g1(root)，行星轮用 external 挂在 g1 上并给 bearing 均布，内齿圈用 internal 挂在任意一个行星轮上、不要给 bearing。齿数须同时满足两个条件，否则只有挂载齿圈的那个行星轮能正确啮合，其余会与齿圈干涉：
   ...
   - 装配条件：(z太阳轮 + z齿圈) 必须能被行星轮数 N 整除（N=3 时和值为 3 的倍数；N=4 时为 4 的倍数）
   - 所有行星轮齿数必须相同。
   - 推荐组合：N=3 取 z太阳=18, z行星=12, z齿圈=42（和 60÷3=20 ✓）；N=4 取 z太阳=20, z行星=20, z齿圈=60（和 80÷4=20 ✓）；N=4 也可 z太阳=16, z行星=14, z齿圈=44（和 60÷4=15 ✓）。
